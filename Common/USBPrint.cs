...
namespace Common
{
    public class USBPrint
    {
        ...
        public static void Print(System.Drawing.Printing.PrintPageEventArgs e, List<Products> list,
            string serialNumber, string salesperson)
        {
            ...
            //打印标题
            e.Graphics.DrawString("    英莱特超市", titlefont, Brushes.Blue, left + 10, top - 5, new StringFormat());
            //打印标题
            e.Graphics.DrawString("      购物凭证", titlefont, Brushes.Blue, left + 10, top + 15, new StringFormat());
            ...
            for (int i = 1; i <= list.Count; i++)
            {
                ...
                decimal unitPrice = list[i - 1].UnitPrive * Convert.ToDecimal(1.00);
                ...
                if (list[i - 1].UnitPrive.ToString("F2").Length == 1)
                    sp2 = "        ";
                else if (list[i - 1].UnitPrive.ToString("F2").Length == 2)
                    sp2 = "       ";
                else if (list[i - 1].UnitPrive.ToString("F2").Length == 3)
                    sp2 = "     ";
                else if (list[i - 1].UnitPrive.ToString("F2").Length == 4)
                    sp2 = "    ";
                else if (list[i - 1].UnitPrive.ToString("F2").Length == 5)
                    sp2 = "   ";
                else if (list[i - 1].UnitPrive.ToString("F2").Length == 6)
                    sp2 = "  ";
                else if (list[i - 1].UnitPrive.ToString("F2").Length > 6)
                    sp2 = " ";
            ...
            e.Graphics.DrawString("在7日内凭小票可开具购物发票", font, Brushes.Black, left, (int)top + 110 + 10 + font.GetHeight(e.Graphics) * 5 + 24, new StringFormat());
            e.Graphics.DrawString("            欢迎再次光临！", font, Brushes.Black, left, (int)top + 110 + 10 + font.GetHeight(e.Graphics) * 7 + 24, new StringFormat());
        }
    }
}
